...
- **R3/R4 (Phase 0 实施 gate)**: R3 NO-GO(T8 锁错层级 → 补 char_normalize_allowed_tools_rejects_mcp_name;doctor 锁定显式移 Phase 1)→ R4 GO,commit `90c8c91`。
- **R5/R6 (Track R 实施 gate)**: R5 NO-GO(1 P1: tokio::join! 把写失败拖到全局 timeout → select 状态机 run_round_trip,write-fails-first 短路 + response-wins 裁定;3 P2: notify 失败 kill / stderr 改 Stdio::null / 前导空格测试)→ R6 GO,commit `1ba776e`。
- **R7/R8/R9 (Track C1 实施 gate,新 codex 线程 019ea12b)**: R7 NO-GO(P1: 碰撞 _2 后缀是假路由 — routes 与 runtime tool_index 都按相同 qualified_name 做 key;P2: --allowedTools 与广告不一致留 C2 + approval 需 raw server identity)→ 修 first-wins+drop + McpRoute{qualified,server_name(raw)} → R8 又 NO-GO(P1: catalog first-wins ≠ runtime tool_index last-writer-wins,广告 metadata 与执行目标错位)→ 改 **last-wins 对齐 manager**(同一 Vec 顺序 → 幸存者天然一致)→ **R9 GO**。**C2 必须**: ① 广告过滤与 dispatch 过滤同一套动态 allowlist 语义(T8);② approval/trust 用 McpRoute.server_name(raw),不从名字反推。
